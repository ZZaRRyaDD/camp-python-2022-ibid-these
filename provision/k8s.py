...
def login(context):
    ...
    context_name = "teleport.saritasa.rocks-teleport.saritasa.rocks"
    context.run(f"kubectl config use-context {context_name}")
...
def logs(context, component=COMPONENT):
...
@task
def python_shell(context, component=COMPONENT):
    """Enter into python shell."""
    execute(context, component=component, entry="shell_plus")


@task
def health_check(context, component=COMPONENT):
